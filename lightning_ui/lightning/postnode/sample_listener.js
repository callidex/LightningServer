...
server.on('message', (msg, rinfo) => {
  ...
  if (data[3] === 0 || data[3] === 4) { // Changed double equals (==) to triple equals (===)
...
async function storeSample(tempObject) {
  ...
  try {
    const smoothed = smoothed_z_score(tempObject.data);
    const res = await client.query(
      `INSERT INTO sample (detector, data, dmatime, batchid, rtsecs, adcseq, smoothed)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [tempObject.detector, tempObject.data, tempObject.dmatime, tempObject.batchid, tempObject.rtsecs, tempObject.adcseq, smoothed]
    );
  } catch (err) {
